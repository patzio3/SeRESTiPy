...
def perform(hosts_list, json_data, nCycles):
        ...
        for i in range(len(hosts_list)):
            _ = communicator.requestEvent("DELETE", [hosts_list[i] for j in range(len(systemnames) * nCycles)], list(range(len(systemnames) * nCycles)))
...
if __name__ == "__main__":
    ...
    nCPU, nRAM, nNodes, nWorkerPerNode = cluster.determineSettings(nSystems, int(sys.argv[2]), int(sys.argv[3]))
    cluster.run(perform, nCPU, nRAM, nNodes, nWorkerPerNode, "LYRA1", json, int(sys.argv[4]))
